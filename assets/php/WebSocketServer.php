<?php
require_once('SILSocketUser.php');

abstract class WebSocketServer {
	protected $userClass = 'SILSocketUser';
	protected $maxBufferSize;
	protected $master;
	protected $sockets = array();
	protected $users = array();
	protected $mensajesRetenidos = array();
	...
	function __construct($addr, $port, $bufferLength = 2048) {
		...
		$this->stdout('SILSocket ha iniciado...');
	}

	abstract protected function proceso($user, $message); // Llamado inmediatamente cuando el dato es recibido.
	abstract protected function conectado($usuario); // Llamado después de que el handshake es enviado al cliente.
	abstract protected function cerrado($usuario); // Llamado después de que la conexión es cerrada.
	abstract protected function conectando($usuario); // Llamado cuando la instancia es creada, antes del handshake

	protected function send($usuario, $mensaje) {
		if ($usuario->handshake) {
			$mensaje = $this->frame($mensaje, $usuario);
			$result = @socket_write($usuario->socket, $mensaje, strlen($mensaje));
		}
		else // Sí aún no hay handshake, retengo el mensaje para enviarlo posteriormente
			$this->mensajesRetenidos[] = array('user' => $usuario, 'message' => $mensaje);;
	}

	protected function tick() {
		// Establecer aquí un proceso periódico que se estará ejecutando, al menos una vez por segundo
	}

	protected function _tick() {
		// Nucleo del mantenimiento básico, como reintentar mensajes fallidos.
		foreach ($this->mensajesRetenidos as $key => $hm) {
			$found = false;
			foreach ($this->users as $currentUser) {
				if ($hm['user']->socket == $currentUser->socket) {
					$found = true;
					if ($currentUser->handshake) {
						unset($this->mensajesRetenidos[$key]);
						$this->send($currentUser, $hm['message']);
					}
				}
			}
			if (!$found) {
				// Si ya no están en la lista de usuarios conectados, borro el mensaje
				unset($this->mensajesRetenidos[$key]);
			}
		}
	}
	...
	public function run() {
		while(true) {
			...
			foreach ($sockets as $socket) {
				if ($socket == $this->master) {
					...
					if (!$cliente) {
						$this->stderr("Error en socket_accept(): " . socket_strerror(socket_last_error()));
						continue;
					}
					else $this->conectar($cliente);
				}
				else {
					...
					if ($numBytes === false) {
						...
						switch ($sockErrNo) {
							...
							case 10054:
								// $this->stderr("Desconexión inusual: " . $socket);
								$this->stderr("Desconexión inusual: " . socket_strerror($sockErrNo) . " - " . $sockErrNo);
								$this->desconectar($socket, true, $sockErrNo); // disconnect before clearing error, in case someone with their own implementation wants to check for error conditions on the socket.
								break;
							default:
								$this->stderr('Error en Socket: ' . socket_strerror($sockErrNo) . " - " . $sockErrNo);
								$this->desconectar($socket, true, $sockErrNo);
						}
					}
					elseif ($numBytes == 0) {
						$this->desconectar($socket);
						$this->stderr("Cliente desconectado. Se perdió la conexión TCP:");
					}
					else {
		...
		protected function conectar($socket) {
			$user = new $this->userClass(uniqid('u'), $socket);
			$this->users[$user->id] = $user;
			$this->sockets[$user->id] = $socket;
			$this->conectando($user);
		}

	protected function desconectar($socket, $triggerClosed = true, $sockErrNo = null) {
	...
	// Verifica si el paquete tiene más de un frame y los procesa cada uno indivivualmente
	protected function split_packet($length,$packet, $user) {
		// Añado un PartialPacket y calculo el nuevo tamaño ($length)
		...
		while($frame_pos<$length) {
			...
			// Divide el frame del paquete y lo procesa
			$frame=substr($fullpacket, $frame_pos, $framesize);

			if (($mensaje = $this->deframe($frame, $user, $headers)) !== FALSE) {
				if ($user->hasSentClose) $this->desconectar($user->socket);
				else {
					if ((preg_match('//u', $mensaje)) || ($headers['opcode'] == 2))
						$this->proceso($user, $mensaje);
					else $this->stderr("Error en UTF-8\n");
				}
			}

			//Obtengo la nueva posición y también modifico el packet data
			$frame_pos += $framesize;
	...
	protected function deframe($message, &$user) {
		...
		if ($willClose) {
			// Falló la conexión, aquí se puede implementar algo
			return false;
		}
